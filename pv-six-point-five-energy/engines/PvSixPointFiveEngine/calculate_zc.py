...
class CalculateZC(object):
    ...
    #     branch_num：组串总数
    #     result_num：逆变器有多少组串
    #     计算整个场站的理论发电量, 组串、逆变器理论发电量
    #     :return: 整个场站的理论发电量, 组串、逆变器理论发电量
    #     """
    ...
    #     n_theory_energy_result = dict()  # 汇流箱论发电量
    #     for c_i, value in result_num.items():
    #         n_theory_energy_result[c_i] = round(branch_theory_energy * value, 2)
    #
    #     return theory_energy_result, b_theory_result, n_theory_energy_result
    ...
    def Theory_Energy(self, branch_num, result_num, b_theory_result, station_id, power, power_josn_path, power_no):
        ...
        :return: 整个场站的理论发电量, 组串、逆变器理论发电量
        """
        energy_theory = Theory_station_energy(power, power_josn_path, power_no, station_id)  # 场站理论发电量
        energy_theory = round(energy_theory, 2)
        theory_energy_result = {"theory_energy_result": energy_theory}
        branch_theory_energy = energy_theory / branch_num  # 组串理论发电量
        branch_theory_energy = round(branch_theory_energy, 2)
    ...
    def Actual_power(self, current, voltage, station_id, df_dcpower, inverterurls, power_no):
        ...
        pv_station：场站容量
        计算理论、实际发电量
        :return: result(场站实际发电量), result_n(逆变器实际发电量), result_b(组串实际发电量),
        theory_energy_result(场站理论发电量)，b_theory_energy_result(组串理论发电量), n_theory_energy_result(逆变器理论发电量)
        """
        df = pd.merge(current, voltage, on=['time', 'branch_no', 'device_code'], how='left')
        ...
        for comb_name in head_list:
            sum_n = 0
            df_box = df.iloc[:, :][df.device_code == comb_name]  # 取出逆变器（如取出GS_XP_13_001_01）所有信息
            if df_box.empty:
                continue
            else:
                b_list = list(set(df_box["branch_no"].values.tolist()))
                N = len(b_list)
                result_num[comb_name] = N
                branch_num = branch_num + N
                for i in b_list:
                    if isinstance(df_box.branch_no.values[0], str):
                        branch_i = df_box.iloc[:, :][df_box.branch_no == str(i)]
                    else:
                        branch_i = df_box.iloc[:, :][df_box.branch_no == i]  # 取出组串所有信息
                    ...
                    sum_b = round(sum_b, 2)
                    result_b[s_d] = sum_b
                    sum_n = sum_n + sum_b  # 计算当前逆变器发电量
                result_n[comb_name] = round(sum_n, 2)
            sums = sums + sum_n  # 计算整个场站发电量
        result["station_id"] = round(sums, 2)
        b_theory_result = result_b.copy()
        theory_energy_result, b_theory_energy_result, n_theory_energy_result = self.Theory_Energy(
            branch_num,result_num,b_theory_result,station_id,df_dcpower,inverterurls,power_no)
        return result, result_n, result_b, theory_energy_result, b_theory_energy_result, n_theory_energy_result

    def Failure_Analysis(self, path, path1, power_zhi, result_n, station_id, d_time):
        """
        :param path:汇流箱json
        :param path1:逆变器json
        ...
        for key_id, value in combiner_dict.items():
            ...
            for i in range(0, len(value), 2):
                if value[i] in result_n.keys():
                    sums = sums + result_n[value[i]]
                    combiner_value.append(result_n[value[i]])
                else:
                    sums = sums + 0
                    results = results.append(
                        pd.DataFrame({'d_time': [d_time], 'details': ['逆变器未接入'], 'dev_name': [value[i + 1]],
                                      'dev_no': [value[i]], 'dev_type': ['逆变器'], 'power': [-1],
            ...
            if abs(Inverter_sum[Inverter_dict[key_id]] - power_zhi[Inverter_dict[key_id]]) < 10:
                ...
                for j in range(len(combiner_value)):
                    if combiner_value[j] < 1 and combiner_value[j] != -1:
                        results = results.append(
                            pd.DataFrame({'d_time': [d_time], 'details': ['逆变器采集器故障'], 'dev_name': [value[2*j + 1]],
                                          'dev_no': [value[2*j]], 'dev_type': ['逆变器'], 'power': [combiner_value[j]],
                                          'station_id': [station_id]}), ignore_index=False)
        return results
